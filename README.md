...
Style rules: markdown and YAML lines wrap at **120 characters**, enforced by `markdownlint` and `yamllint` in
pre-commit. The same 120 columns apply to shell as a convention — `.editorconfig` sets `max_line_length = 120` for
every file type, but no hook enforces it for `*.sh`, so your editor is the only thing keeping you honest there. Shell
must pass `shellcheck` and `shfmt`; never interpolate `${{ }}` expressions into a `run:` body — pass them through
`env:` instead.
